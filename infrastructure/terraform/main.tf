...
module "cloudflare_api_token_parameter" {
  ...
  description = "Cloudflare API token for serverless-node-dynamodb-ui Terraform"
}
...
module "ga_id_parameter" {
  ...
  description = "Google Analytics ID injected into serverless-node-dynamodb-ui at build time"
}
...
module "codebuild_deploy_project" {
  ...
  name                               = var.codebuild_project_name
  description                        = "Build and deploy serverless-node-dynamodb-ui to S3 + CloudFront"
  ...
  environment_variables = [
    ...
    { name = "API_BASE_URI", value = var.api_base_uri },
  ]
}
